...
const NewArrivals = () => {
  ...
  const arrowBtn = (enabled) =>
    `p-2 rounded-full ${
      enabled ? "bg-white text-naqsh-gold border border-naqsh-sand hover:border-naqsh-gold" : "bg-naqsh-sand/40 text-naqsh-umber/30 cursor-not-allowed"
    ...
    return (
      <section className="py-16 md:py-24 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto text-center">
          <h2 className="font-display text-3xl md:text-4xl text-naqsh-noir mb-4">New Arrivals</h2>
          {loading && <div className="animate-spin rounded-full h-10 w-10 border-b-2 border-naqsh-gold mx-auto"></div>}
          {error && <p className="text-red-600">Error: {error}</p>}
          {!loading && !error && <p className="text-naqsh-umber/60">No new arrivals available at the moment.</p>}
        </div>
      </section>
    );
  }
    ...
    <section className="py-16 md:py-24 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12 relative">
          <h2 className="font-display text-3xl md:text-4xl text-naqsh-noir mb-4">New Arrivals</h2>
          <p className="text-lg text-naqsh-umber/70 max-w-2xl mx-auto">
          ...
          {newArrivals.map((product) => {
            ...
            return (
              <div key={productId} className="flex-shrink-0 w-64 sm:w-72 relative group">
                <div className="relative overflow-hidden rounded-sm aspect-[4/5] bg-naqsh-sand/30">
                  {mainImage.url ? (
                    <img
                      src={mainImage.url}
                    ...
                    <div className="w-full h-full flex items-center justify-center">
                      <span className="text-naqsh-umber/40">No Image</span>
                    </div>
                  )}
                </div>
                <div className="mt-4 text-center">
                  <Link to={`/product/${productId}`} className="block group-hover:text-naqsh-gold transition-colors duration-200">
                    <h3 className="font-display text-sm uppercase tracking-wide text-naqsh-noir">{product.name}</h3>
                    <p className="text-naqsh-gold mt-1">Rs. {product.price?.toFixed(2)}</p>
                  </Link>
                </div>
